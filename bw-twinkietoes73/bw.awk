...
G["path"]  = "/home/greenc/toolforge/bw-twinkietoes73/"
G["email"] = readfile("/home/greenc/toolforge/scripts/secrets/bw-twinkietoes73.email")
...
G["api agent"] = "Backlinks Watchlist (User:Green_C on en)"
...
function main(cfgfile		,V ,name, br) {
        ...
        while ((getline name < cfgfile) > 0) {
            ...
            if ( V["newflag"] ) {
                close(V["addtxt"])
                close(V["subtxt"])
                V["command"] = sprintf("mailx -s 'New Backlinks at Wikipedia ('%s')' -- %s", shquote(V["name"]), G["email"])
                print V["emailtxt"] | V["command"]
                close(V["command"])
            }
        }   
...
function files_verify(files_system, files_local, localdir,
	...
	missing = 0
        ...
        if ( missing ) {
            if ( G["email"] ~ "@" ) {
                command = sprintf("mailx -s \"Error in Backlinks Watchlist\" -- %s", G["email"])
